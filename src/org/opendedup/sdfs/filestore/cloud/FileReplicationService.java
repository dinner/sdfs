...
public class FileReplicationService {
			...
			while (!done) {
				try {
					...
					if (!isSymlink && f.isDirectory())
						fn = fn + DM;
					this.sync.deleteFile(fn, "files");//删除云端文件
					done = true;
				} catch (Exception e) {
					if (tries > maxTries)
						throw e;
	...
	public void downloadAll(CloudSyncDLRequest req) {
		...
		try {
			...
			DDBDownloader.reset();
			System.out.println("卷id是："+req.getVolumeID());
			...
			while (fname != null) {
				...
				System.out.println("文件名encrypt:"+efs);
				if (this.sync.isCheckedOut("files/" + efs, req.getVolumeID())) {//检测在claims下面是否有这个文件
					File f = new File(Main.volume.getPath() + File.separator + fname);
