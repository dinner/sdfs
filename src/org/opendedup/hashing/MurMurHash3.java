...
public final class MurMurHash3 {
	...
	public static final long getLongLittleEndian(byte[] buf, int offset) {
		return ((long) buf[offset + 7] << 56) // no mask needed
				...
				| ((buf[offset + 1] & 0xffL) << 8) | ((buf[offset] & 0xffL)); // no
																				// shift
																				// needed
	}
	...
	public static void murmurhash3_x64_128(byte[] key, int offset, int len,
			int seed, LongPair out) {
	...
	/** Returns the MurmurHash3_x64_128 hash, placing the result in "out". */
	public static byte[] murmurhash3_x64_128(byte[] key, int seed) {
		// The original algorithm does have a 32 bit unsigned seed.
		// We have to mask to match the behavior of the unsigned types and
		// prevent sign extension.
		long h1 = seed & 0x00000000FFFFFFFFL;
		...
		h1 += h2;
		h2 += h1;
		byte[] hsh = new byte[16];
		ByteBuffer b = ByteBuffer.wrap(hsh).order(ByteOrder.LITTLE_ENDIAN)
				.putLong(h1).putLong(h2);
		return b.array();
	}
